...
// worldwide google domains in map for searching.
...
//type of result which will required
...
//supporting agent like browsers
...
//defining random fuction to get random useragent.
...
//function to build google query that need to be passed in search bar
func buildGoogleUrls(searchTerm, countryCode, languageCode string, pages, count int)([]string, error){
	toScrape := []string{}
	searchTerm = strings.Trim(searchTerm, " ") // trimming whitespace
	searchTerm = strings.Replace(searchTerm, " ", "+", -1) // concatenate space by +
	if googleBase, found := googleDomains[countryCode]; found{
		for i :=0; i<pages ; i++{  //for number of pages to be displayed as result 
			start := i*count   
...
doc, err := goquery.NewDocumentFromResponse(response) //parsing result into new document
...
func main(){
	res, err := GoogleScrape("somraj bishnoi","com","en", nil, 1, 30, 10)
	if err == nil{
		for _, res := range res{
			fmt.Println(res)
		}
